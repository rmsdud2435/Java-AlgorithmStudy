 ...
 * 문제 설명
 * 
 * 2차원 좌표 평면에 다양한 형태의 직사각형들이 놓여 있습니다. 모든 직사각형은 X축 위, Y축 오른쪽에 존재하며, 직사각형의 모든 변은 X축 또는 Y축에 평행합니다. 
 * 이 직사각형들을 아래 규칙에 따라 이동하려 합니다.
 * 
 ...
 * 직사각형은 X축, 또는 Y축에 닿으면 더는 움직이지 않으며, 각 직사각형은 다른 직사각형을 뚫거나 통과해서 움직일 수 없습니다. 
 * 또, 모든 직사각형은 회전이나 기울임 없이 X축, Y축에 평행하게 움직여야 합니다.
 * 
 ...
 * rect_move_3.png
 * 
 * 위 그림이 모든 직사각형을 아래로 한 번, 왼쪽으로 한 번 이동시킨 최종 결과입니다. 
 * 위 예시에서 G번 직사각형의 경우 아래쪽 방향으로 더 움직일 수 있는 상태이나, 본 문제에서는 모든 직사각형을 아래로 한 번, 왼쪽으로 한 번만 이동시킨 결과를 구하면 됩니다.
 * 
 * 직사각형이 담긴 2차원 배열 rectangles가 매개변수로 주어질 때, 
 * 직사각형들을 문제에 주어진 조건대로 이동시킨 후 각 직사각형의 위치를 배열에 담아 return 하도록 solution 함수를 완성해주세요.
 * 
 * 
 * 제한사항
 * 
